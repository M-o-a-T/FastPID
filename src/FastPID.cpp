...
bool FastPID::setCoefficients(uint32_t kp, uint32_t ki, uint32_t kd, uint32_t hz) {
  _p = kp;
  _i = ki / hz;
  _d = kd * hz;
  return ! _cfg_err;
...
bool FastPID::configure(uint32_t kp, uint32_t ki, uint32_t kd, uint32_t hz, int bits, bool sign) {
  clear();
  _cfg_err = false;
